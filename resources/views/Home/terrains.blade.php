    ...
    <div class="min-h-[100vh] text-[white] relative z-10  pt-12 md:pt-16">
        ...
        <div class="pagination-container my-8 px-4">
            <nav aria-label="Pagination" class="flex justify-center">
                <ul class="inline-flex items-center -space-x-px rounded-lg bg-[#3b2c2c]/80 shadow-lg overflow-hidden">
                    ...
                    @for ($i = 1; $i <= $terrains->lastPage(); $i++)
                        @if ($i == $terrains->currentPage())
                        <li>
        ...
        <div class="pagination-mobile sm:hidden my-6 px-4">
            <div class="flex justify-between items-center">
                ...
                @else
                <a href="{{ $terrains->previousPageUrl() }}" class="inline-flex items-center px-4 py-2 text-sm bg-[#3b2c2c] text-gray-300 rounded-lg hover:bg-[#580a21]/70 hover:text-white transition-colors duration-300">
                    <svg aria-hidden="true" class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" d="M7.707 14.707a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l2.293 2.293a1 1 0 010 1.414z" clip-rule="evenodd"></path>
                    </svg>
                    Précédent
                ...
                <span class="text-sm text-gray-300">Page {{ $terrains->currentPage() }} sur {{ $terrains->lastPage() }}</span>

                @if ($terrains->hasMorePages())
                <a href="{{ $terrains->nextPageUrl() }}" class="inline-flex items-center px-4 py-2 text-sm bg-[#3b2c2c] text-gray-300 rounded-lg hover:bg-[#580a21]/70 hover:text-white transition-colors duration-300">
                    Suivant
