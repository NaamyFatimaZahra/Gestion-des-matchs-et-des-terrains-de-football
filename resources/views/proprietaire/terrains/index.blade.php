...
<div class="container mx-auto px-4 py-8 mt-[4rem] text-gray-800">
    ...
    <a href="{{ route('proprietaire.terrain.create') }}" class="bg-[#580a21] hover:bg-[#420718] text-white px-4 py-2 rounded-lg flex items-center">
...
<!-- Message d'erreur général -->
   ...
   @endif
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-rose-50">
                    <tr>
                        ...
                        <th scope="col" class="px-10 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                            Terrain
                        </th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                            Propriétaire
                    ...
                    <tr class="hover:bg-rose-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <form action="{{ route('proprietaire.terrain.update-status', $terrain->id) }}" method="POST">
                                ...
                                <select name="status" onchange="this.form.submit()" class="text-xs font-semibold rounded border-0 px-2 py-1
                                    ...
                                    ($terrain->status === 'occupé' ? 'bg-[#580a21] text-white' : 
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 py-1 text-xs font-semibold rounded
                                ...
                                ($terrain->admin_approval === 'rejete' ? 'bg-[#580a21] text-white' : 
    ...
    <div class="flex justify-end space-x-3">
        <a href="{{ route('proprietaire.terrain.show', $terrain->id) }}" class="text-gray-600 hover:text-[#580a21]" title="Voir détails">
            <i class="fas fa-eye"></i>
        </a>
        <a href="{{ route('proprietaire.terrain.edit', $terrain->id) }}" class="text-blue-600 hover:text-blue-800" title="Modifier">
            <i class="fas fa-edit"></i>
        </a>
        <form action="{{ route('proprietaire.terrain.destroy', $terrain->id) }}" method="POST" class="inline" >
            @csrf
            @method('DELETE')
            <button type="submit" class="text-[#580a21] hover:text-[#420718]" title="Supprimer">
                <i class="fas fa-trash"></i>
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="px-6 py-4 text-center text-sm text-gray-600">
