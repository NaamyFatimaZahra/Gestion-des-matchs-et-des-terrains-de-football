...
<div class="min-h-screen bg-rose-50 text-gray-800">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 space-y-4 md:space-y-0">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('proprietaire.terrain.edit', $terrain->id) }}" class="bg-[#580a21] hover:bg-[#420718] text-white px-5 py-2 rounded-lg text-sm font-medium flex items-center transition-colors duration-200 shadow-md">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
            ...
            <div class="lg:col-span-4 bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200 transition-transform duration-300 hover:shadow-2xl">
                ...
                <div class="relative">
                    ...
                    <div class="absolute top-4 right-4 flex flex-col space-y-2">
                        <span class="bg-opacity-80 bg-white text-gray-800 text-xs px-3 py-1 rounded-full flex items-center">
                            <span class="w-2 h-2 rounded-full mr-2 
                {{ $terrain->status === 'disponible' ? 'bg-emerald-500' : 
                ($terrain->status === 'occupé' ? 'bg-rose-500' : 
                ($terrain->status === 'maintenance' ? 'bg-amber-500' : 'bg-gray-400')) }}"></span>
            ...
            ($terrain->status === 'maintenance' ? 'Maintenance' : 'En attente')) }}
                        ...
                        <span class="bg-opacity-80 bg-white text-gray-800 text-xs px-3 py-1 rounded-full flex items-center">
                            <span class="w-2 h-2 rounded-full mr-2 
                {{ $terrain->admin_approval === 'approuve' ? 'bg-emerald-500' : 
                ($terrain->admin_approval === 'rejete' ? 'bg-red-500' : 
                ($terrain->admin_approval === 'suspended' ? 'bg-orange-500' : 'bg-amber-500')) }}"></span>
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-center">
                        <div class="bg-rose-50 rounded-lg p-3">
                            <span class="block text-sm text-gray-600 mb-1">Réservations</span>
                            <span class="block text-xl font-bold text-gray-800">{{ $terrain->reservation_count }}</span>
                        </div>
                        <div class="bg-rose-50 rounded-lg p-3">
                    ...
                    <div class="lg:flex">
                        <div class="lg:w-2/3 p-6">
                            <h3 class="text-xl font-semibold mb-4 flex items-center text-gray-800">
                                <svg class="w-5 h-5 mr-2 text-[#580a21]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200 mb-8">
            <div class="p-6 border-b border-gray-200 flex justify-between items-center">
                ...
                <a href="" class="bg-[#580a21] hover:bg-[#420718] text-white text-sm px-4 py-2 rounded-lg flex items-center transition-colors duration-200">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-rose-50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">ID</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Terrain</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Client</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Date</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Heure début</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Heure fin</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Durée (H)</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Statut</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-rose-50 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <div class="flex items-center">
                                        <div class="w-8 h-8 rounded-full bg-rose-50 flex items-center justify-center mr-2">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    ...
                                    <form action="{{ route('proprietaire.reservation.update-status', $reservation->id) }}" method="POST">
                                        @csrf
                                        @method('patch')
                                        <select name="status" onchange="this.form.submit()" class="border rounded px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-[#580a21] 
                                        {{ $reservation->status == 'confirmee' ? 'bg-emerald-500 text-white border-emerald-400' : 
                                           ($reservation->status == 'en_attente' ? 'bg-amber-500 text-white border-amber-400' : 
        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-rose-50 p-5 rounded-lg">
                    <h4 class="text-lg font-medium mb-4 text-gray-800">Modifier le statut du terrain</h4>
                    <form action="{{ route('proprietaire.terrain.update-status', $terrain->id) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PATCH')
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label for="status" class="block text-sm font-medium text-gray-600 mb-2">Statut opérationnel</label>
                                <select id="status" name="status" class="bg-white text-gray-800 border border-gray-300 rounded-lg px-4 py-3 w-full focus:outline-none focus:ring-2 focus:ring-[#580a21] focus:border-transparent transition-all duration-200">
                            ...
                            <div>
                                <button type="submit" class="bg-[#580a21] hover:bg-[#420718] text-white px-5 py-3 rounded-lg text-sm font-medium w-full flex items-center justify-center transition-colors duration-200 shadow-md">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="bg-rose-50 p-5 rounded-lg">
                    <h4 class="text-lg font-medium mb-4 text-gray-800">Supprimer ce terrain</h4>
                    <p class="text-sm text-gray-600 mb-4">Attention: La suppression est irréversible et supprimera toutes les réservations associées à ce terrain.</p>
            ...
            <div class="p-6">
                ...
                <div class="border-b border-gray-200 pb-5 mb-5 last:border-b-0 last:mb-0 last:pb-0">
                    <div class="flex justify-between mb-3">
                        <div class="flex items-center">
                            <div class="w-10 h-10 rounded-full bg-rose-50 flex items-center justify-center mr-3">
                                <svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                </svg>
                            </div>
                            <div>
                                <div class="font-medium text-gray-800">{{ $comment->user->name ?? 'Client Anonyme' }}</div>
