...
<section class="w-[100%] min-h-[100vh] relative">
    ...
    <div class="min-h-[100vh] text-[white] relative z-10 pt-16">
        <header class="bg-[#580a21] py-20 text-center">
            ...
            <h1 class="text-4xl font-bold uppercase"> Request</h1>
        </header>
        ...
        <div class="flex-1 p-4 md:p-6 lg:p-8 min-h-[60vh]">
            <div class="max-w-7xl mx-auto">
                        ...
                        <tbody class="bg-[#685f5fe8] text-white divide-y divide-gray-600/30">
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-white">{{ $request->squads[0]->name_squad }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <img
                                            src="{{ isset($request->pivot->profile_picture) ? asset('storage/'. $request->pivot->profile_picture) : asset('storage/profile-pictures/blank-profile.webp') }}"
                                            alt="pivot image"
                                            class="w-8 h-8 rounded-full border-solid border-2 border-[#580a21] mr-3"
                                        />
                                        <div class="text-sm text-white">{{ $request->name }}</div>
                                    </div>
                                </td>
                              ...
                              <td class="py-4 whitespace-nowrap text-sm font-medium flex gap-8">
                                <form action="{{route('joueur.requests.update') }}" method="POST" class="flex items-center space-x-2">
                                    ...
                                    <select name="response" required onchange="this.form.submit()" class="bg-[#423939] text-white text-xs rounded-md border-none focus:outline-none focus:ring-1 focus:ring-[#580a21] py-1 px-2">
                                        <option value="accepté" {{ $request->pivot->acceptationUser === 'accepté' ? 'selected' : '' }}>Accepter</option>
                                        <option value="refusé" {{ $request->pivot->acceptationUser === 'refusé' ? 'selected' : '' }}>Refuser</option>
                                        <option value="en attente" {{ $request->pivot->acceptationUser === 'en attente' ? 'selected' : '' }} hidden>En attente</option>
                                    </select>
                                    <input type="text" name="userId" value="{{ $request->pivot->user_id }}" class="hidden">
                                    <input type="text" name="squadId" value="{{ $request->pivot->squad_id }}" class="hidden">
                                </form>
                                 <!-- Icône œil avec lien vers joueur.squad.show -->
                                    <a href="{{ route('joueur.squad.show', $request->squads[0]->id) }}"
                ...
                @else
                <!-- Message quand il n'y a pas de requests -->
                <div class="col-span-1 sm:col-span-2 lg:col-span-3 p-8 bg-[#685f5fe8] rounded-xl shadow text-center">
                    ...
                    <p class="text-[#d8d9dd] text-xl font-bold mb-2">Aucune request</p>
                    <p class="text-[#d8d9dd] mb-4">Vous n'avez aucune request de squad en attente pour le moment.</p>
